...
class Canvas(QWidget):
    ...
    annotateRelation = pyqtSignal(Relationship)
    ...
    def relating(self):
        return self.rel_mode == 1

    def set_relating(self , value = True):
    ...
    def mousePressEvent(self, ev):
        pos = self.transform_pos(ev.pos()) #获取当前当前鼠标坐标

        if ev.button() == Qt.LeftButton:  #鼠标左键处理事件
            if self.drawing():      #  如果是创建模式
                self.handle_drawing(pos)
            else:        #  如果是编辑模式
                selection = self.select_shape_point(pos) #根据点返回选中的形状
                self.prev_point = pos  #将当前点存到prev_point中

                if selection is None:
                    # pan
                    QApplication.setOverrideCursor(QCursor(Qt.OpenHandCursor)) #设置应用程序的光标形状为打开手的形状
                    self.pan_initial_pos = ev.pos()  # 存储鼠标按下时的初始位置，用于平移操作。
                    self.prev_shape = None

                if self.relating() and  selection is not None:
                    self.prev_shape = selection

        elif ev.button() == Qt.RightButton and self.editing():   #鼠标邮件处理事件  同时时是编辑模式
            ...
            if self.relating():
                if self.prev_shape:
                    ...
                    if selection :
                        if isinstance(self.prev_shape , Shape) and isinstance(selection , Shape):
                            relationship = Relationship(self.prev_shape , selection , self.relation_id)
                            self.relationships.append(relationship)
                            self.relation_id += 1
                            print(self.relation_id)
                            self.annotateRelation.emit(relationship)

        self.update()

    def mouseReleaseEvent(self, ev):
        if ev.button() == Qt.RightButton:  #定义处理右键释放事件
            ...
            if not menu.exec_(self.mapToGlobal(ev.pos())) \
                ...
                self.selected_shape_copy = None
                self.repaint()
        elif ev.button() == Qt.LeftButton and self.selected_shape:  #定义左键处理事件
            if self.selected_vertex():
                self.override_cursor(CURSOR_POINT)
            else:
                self.override_cursor(CURSOR_GRAB)
    ...
    def handle_drawing(self, pos):
        if self.current and self.current.reach_max_points() is False:   #当前存在绘制的图形、切没有达到最大点数
            init_pos = self.current[0]
            min_x = init_pos.x()
            min_y = init_pos.y()
            ...
            self.current = Shape()
            #
            self.current.shape_id = self.object_id
            self.object_id += 1
            #
    ...
    def select_shape_point(self, point):
        ...
        self.de_select_shape()  #清除当前选定的任何形状。
        if self.selected_vertex():  # A vertex is marked for selection.
            # 这一部分检查是否有顶点被标记为选择状态。如果存在选定的顶点，它会突出显示该顶点，然后选择包含该顶点的形状，并返回该顶点的索引。
...
if __name__ == '__main__':
    ...
    canvas = Canvas()
    canvas.show()
    filename = r'E:\360MoveData\Users\qiuhao\Desktop\test\data\000001.jpg'
    reader = QImageReader(filename)
    image = reader.read()
    canvas.load_pixmap(QPixmap.fromImage(image))
    sys.exit(app.exec_())